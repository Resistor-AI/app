...
import {
  ...
  withSpring, cancelAnimation, Easing, runOnJS,
...
export function useStaggeredEntry(options: StaggeredEntryOptions = {}) {
  const {
    index = 0, staggerDelay = 100, maxDelay = 500, duration = 400,
    useSpring = false, springPreset = "gentle",
    translateY: initialTranslateY = 20, initialScale = 0.95,
    trigger = true, onComplete,
  } = options;
  ...
  const triggerEntry = useCallback(() => {
    ...
    if (useSpring) {
      ...
      scale.value = withDelay(delay, withSpring(1, springConfig, (finished) => {
        if (finished && onComplete) runOnJS(onComplete)();
      }));
    } else {
      translateY.value = withDelay(delay, withTiming(0, timingConfig));
      scale.value = withDelay(delay, withTiming(1, timingConfig, (finished) => {
        if (finished && onComplete) runOnJS(onComplete)();
      }));
    }
  }, [reduceMotion, delay, duration, useSpring, springPreset, onComplete, opacity, translateY, scale]);
  ...
  useEffect(() => { if (trigger) triggerEntry(); }, [trigger, triggerEntry]);
  ...
  return { animatedStyle, triggerEntry, reset };
}
